...
func TestAllWithValidEvents(t *testing.T) {
	...
	store.On("GetEvents", mock.Anything, "b").Return([]*storage.StoredEvent{
		&storage.StoredEvent{
			...
			Timestamp: ddd.NewTimestamp(),
			Type:      "ClusterCreated",
			Data:      `{"Timestamp":0,"UUID":"b","Name":"name","Endpoint":"endpoint","Token":"token","Cert":"cert"}`,
		},
		&storage.StoredEvent{
			UUID:      "b",
			Version:   2,
	...
	store.On("GetEvents", mock.Anything, "c").Return([]*storage.StoredEvent{
		&storage.StoredEvent{
			...
			Timestamp: ddd.NewTimestamp(),
			Type:      "ClusterCreated",
			Data:      `{"Timestamp":0,"UUID":"c","Name":"name","Endpoint":"endpoint","Token":"token","Cert":"cert"}`,
		},
		&storage.StoredEvent{
			UUID:      "c",
			Version:   2,
	...
	assert.Equal(t, "endpoint", first.Endpoint())
	assert.Equal(t, "token", first.Token())
	assert.Equal(t, "cert", first.Cert())
	...
	assert.Equal(t, "NEWtoken", second.Token())
	assert.Equal(t, "NEWcert", second.Cert())
...
func TestLoadEmptyEvents(t *testing.T) {
	...
	assert.Nil(t, result)
	assert.Error(t, err)
	assert.Equal(t, ddd.NotFoundError{Entity: "Cluster"}, err)
}
...
func TestLoadDestroyed(t *testing.T) {
	...
	assert.Nil(t, result)
	assert.Error(t, err)
	assert.Equal(t, ddd.DestroyedError{Entity: "Cluster"}, err)
}

func TestSaveValid(t *testing.T) {
	c, _ := Create("a", "name", "endpoint", "token", "cert", ddd.NewTimestamp())
	store := &mockStorage{}
