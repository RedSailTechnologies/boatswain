...
export class DeploymentDialogComponent implements OnInit {
  ...
  constructor(
    public dialog: MatDialogRef<DeploymentDialogComponent>,
    ...
    if (!this.isAdd) {
      this.client.read(<ReadDeployment>{ uuid: data['uuid'] }).then((value) => {
        ...
        this.deploymentForm.controls['filePath'].setValue(
          this.deployment.filePath
        );
      });
    }

    var repoClient = new DefaultRepo(
      `${location.protocol}//${location.host}/api`,
      auth.fetch()
    );
